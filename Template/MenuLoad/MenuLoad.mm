...
//  AsuraADBot
//
//  Created by Euclid Jan Guillermo on 3/1/26.
//
...
{
    NSURL* const AnimatedImageURL = [NSURL URLWithString:@"https://pic1.imgdb.cn/item/69a3fc2bf070dc0f83dee8ed.png"];
    ...
    else
    {
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            @autoreleasepool
            {
                ...
                {
                    [self loadMenuIconFromURL:AnimatedImageURL completion:^(UIImage * _Nullable image) {
                        if (image)
                        {
                            self.CachedGifImage = image;
                            dispatch_async(dispatch_get_main_queue(), ^{
                                [self CreateVisibleMenuButton:image InMainView:InMainView];
                            });
                        }
                    }];
                }
...
- (void)loadMenuIconFromURL:(NSURL *)url completion:(void (^)(UIImage * _Nullable image))completion {
    if (!url) { if (completion) completion(nil); return; }
    
    NSString *cachePath = CacheFilePathForURL(url);
    NSData *data = [NSData dataWithContentsOfFile:cachePath];
    if (!data) {
        // Not cached: download (do this off the main thread)
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            NSData *downloaded = [NSData dataWithContentsOfURL:url];
            if (downloaded.length > 0) {
                [downloaded writeToFile:cachePath atomically:YES];
                ExcludeFromBackup(cachePath);
                UIImage *img = [self ProcessAnimatedImageData:downloaded]; // your GIF-safe builder
                dispatch_async(dispatch_get_main_queue(), ^{
    ...
    // Cached hit
    UIImage *img = [self ProcessAnimatedImageData:data];
    if (completion) completion(img);
}
